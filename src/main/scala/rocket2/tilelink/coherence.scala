...
import rocket2.config.Parameters
import rocket2._
...
trait HasClientSideCoherencePolicy {
  ...
  def clientMetadataOnReset(implicit p: Parameters): ClientMetadata
  def clientMetadataOnHit(cmd: UInt, meta: ClientMetadata): ClientMetadata
  def clientMetadataOnCacheControl(cmd: UInt, meta: ClientMetadata)(implicit p: Parameters): ClientMetadata
  def clientMetadataOnGrant(incoming: Grant, cmd: UInt, meta: ClientMetadata)(implicit p: Parameters): ClientMetadata
  def clientMetadataOnProbe(incoming: Probe, meta: ClientMetadata)(implicit p: Parameters): ClientMetadata
}
...
trait HasManagerSideCoherencePolicy extends HasDirectoryRepresentation {
  ...
  def managerMetadataOnReset(implicit p: Parameters): ManagerMetadata
  def managerMetadataOnRelease(incoming: Release, src: UInt, meta: ManagerMetadata)(implicit p: Parameters): ManagerMetadata
  def managerMetadataOnGrant(outgoing: Grant, dst: UInt, meta: ManagerMetadata)(implicit p: Parameters) =
...
class MICoherence(dir: DirectoryRepresentation) extends CoherencePolicy(dir) {
  ...
  val acquireExclusive :: Nil = Enum(nAcquireTypes)
  val probeInvalidate :: probeCopy :: Nil = Enum(nProbeTypes)
  val releaseInvalidateData :: releaseCopyData :: releaseInvalidateAck :: releaseCopyAck :: Nil = Enum(nReleaseTypes)
  val grantExclusive :: Nil = Enum(nGrantTypes)

  val releaseTypesWithData = VecInit(releaseInvalidateData, releaseCopyData)
  val grantTypesWithData = VecInit(grantExclusive)

  // Client states and functions
  val nClientStates = 2
  val clientInvalid :: clientValid :: Nil = Enum(nClientStates)

  val clientStatesWithReadPermission = VecInit(clientValid)
  val clientStatesWithWritePermission = VecInit(clientValid)
  val clientStatesWithDirtyData = VecInit(clientValid)
  ...
  def getReleaseType(cmd: UInt, meta: ClientMetadata): UInt = {
    ...
    MuxCase(releaseCopyAck, Seq(
      (cmd === M_FLUSH  ) -> Mux(dirty, releaseInvalidateData, releaseInvalidateAck),
      (cmd === M_PRODUCE) -> Mux(dirty, releaseCopyData, releaseCopyAck),
      (cmd === M_CLEAN  ) -> Mux(dirty, releaseCopyData, releaseCopyAck)))
  }

  def getReleaseType(incoming: Probe, meta: ClientMetadata): UInt =
    MuxLookup(incoming.p_type, releaseInvalidateAck, Seq(
      probeInvalidate -> getReleaseType(M_FLUSH, meta),
      probeCopy       -> getReleaseType(M_CLEAN, meta)))

  def clientMetadataOnReset(implicit p: Parameters) = ClientMetadata(clientInvalid)

  def clientMetadataOnHit(cmd: UInt, meta: ClientMetadata) = meta

  def clientMetadataOnCacheControl(cmd: UInt, meta: ClientMetadata)(implicit p: Parameters) =
    ClientMetadata(Mux(cmd === M_FLUSH, clientInvalid, meta.state))

  def clientMetadataOnGrant(incoming: Grant, cmd: UInt, meta: ClientMetadata)(implicit p: Parameters) =
    ClientMetadata(Mux(incoming.isBuiltInType(), clientInvalid, clientValid))

  def clientMetadataOnProbe(incoming: Probe, meta: ClientMetadata)(implicit p: Parameters) =
  ...
  def getProbeType(cmd: UInt, meta: ManagerMetadata): UInt = {
    Mux(cmd === M_FLUSH, probeInvalidate, probeCopy)
  }
  ...
  def managerMetadataOnRelease(incoming: Release, src: UInt, meta: ManagerMetadata)(implicit p: Parameters) = {
...
/** A simple protocol with only three Client states.
  ...
  * Only a single client may ever have a copy of a block at a time.
  */
class MEICoherence(dir: DirectoryRepresentation)(implicit p: Parameters) extends CoherencePolicy(dir) {
  // Message types
  val nAcquireTypes = 2
  val nProbeTypes = 3
  val nReleaseTypes = 6
  ...
  val acquireExclusiveClean :: acquireExclusiveDirty :: Nil = Enum(nAcquireTypes)
  val probeInvalidate :: probeDowngrade :: probeCopy :: Nil = Enum(nProbeTypes)
  val releaseInvalidateData :: releaseDowngradeData :: releaseCopyData :: releaseInvalidateAck :: releaseDowngradeAck :: releaseCopyAck :: Nil = Enum(nReleaseTypes)
  val grantExclusive :: Nil = Enum(nGrantTypes)

  val releaseTypesWithData = VecInit(releaseInvalidateData, releaseDowngradeData, releaseCopyData)
  val grantTypesWithData = VecInit(grantExclusive)

  // Client states and functions
  val nClientStates = 3
  val clientInvalid :: clientExclusiveClean :: clientExclusiveDirty :: Nil = Enum(nClientStates)

  val clientStatesWithReadPermission = VecInit(clientExclusiveClean, clientExclusiveDirty)
  val clientStatesWithWritePermission = VecInit(clientExclusiveClean, clientExclusiveDirty)
  val clientStatesWithDirtyData = VecInit(clientExclusiveDirty)
...
class MSICoherence(dir: DirectoryRepresentation) extends CoherencePolicy(dir) {
  ...
  val acquireShared :: acquireExclusive :: Nil = Enum(nAcquireTypes)
  val probeInvalidate :: probeDowngrade :: probeCopy :: Nil = Enum(nProbeTypes)
  val releaseInvalidateData :: releaseDowngradeData :: releaseCopyData :: releaseInvalidateAck :: releaseDowngradeAck :: releaseCopyAck :: Nil = Enum(nReleaseTypes)
  val grantShared :: grantExclusive :: grantExclusiveAck :: Nil = Enum(nGrantTypes)

  val releaseTypesWithData = VecInit(releaseInvalidateData, releaseDowngradeData, releaseCopyData)
  val grantTypesWithData = VecInit(grantShared, grantExclusive)

  // Client states and functions
  val nClientStates = 3
  val clientInvalid :: clientShared :: clientExclusiveDirty :: Nil = Enum(nClientStates)

  val clientStatesWithReadPermission = VecInit(clientShared, clientExclusiveDirty)
  val clientStatesWithWritePermission = VecInit(clientExclusiveDirty)
  val clientStatesWithDirtyData = VecInit(clientExclusiveDirty)
...
class MESICoherence(dir: DirectoryRepresentation) extends CoherencePolicy(dir) {
  ...
  // Client states and functions
  val nClientStates = 4
  val clientInvalid :: clientShared :: clientExclusiveClean :: clientExclusiveDirty :: Nil = Enum(nClientStates)

  val clientStatesWithReadPermission = VecInit(clientShared, clientExclusiveClean, clientExclusiveDirty)
  val clientStatesWithWritePermission = VecInit(clientExclusiveClean, clientExclusiveDirty)
  val clientStatesWithDirtyData = VecInit(clientExclusiveDirty)
...
class MigratoryCoherence(dir: DirectoryRepresentation) extends CoherencePolicy(dir) {
  ...
  val acquireShared :: acquireExclusive :: acquireInvalidateOthers :: Nil = Enum(nAcquireTypes)
  val probeInvalidate :: probeDowngrade :: probeCopy :: probeInvalidateOthers :: Nil = Enum(nProbeTypes)
  val releaseInvalidateData :: releaseDowngradeData :: releaseCopyData :: releaseInvalidateAck :: releaseDowngradeAck :: releaseCopyAck :: releaseDowngradeDataMigratory :: releaseDowngradeAckHasCopy :: releaseInvalidateDataMigratory :: releaseInvalidateAckMigratory :: Nil = Enum(nReleaseTypes)
  val grantShared :: grantExclusive :: grantExclusiveAck :: grantReadMigratory :: Nil = Enum(nGrantTypes)

  val releaseTypesWithData = VecInit(releaseInvalidateData, releaseDowngradeData, releaseCopyData, releaseInvalidateDataMigratory, releaseDowngradeDataMigratory)
  val grantTypesWithData = VecInit(grantShared, grantExclusive, grantReadMigratory)

  // Client states and functions
  val nClientStates = 7
  val clientInvalid :: clientShared :: clientExclusiveClean :: clientExclusiveDirty :: clientSharedByTwo :: clientMigratoryClean :: clientMigratoryDirty :: Nil = Enum(nClientStates)

  val clientStatesWithReadPermission = VecInit(clientShared, clientExclusiveClean, clientExclusiveDirty, clientSharedByTwo, clientMigratoryClean, clientMigratoryDirty)
  val clientStatesWithWritePermission = VecInit(clientExclusiveClean, clientExclusiveDirty, clientMigratoryClean, clientMigratoryDirty)
  val clientStatesWithDirtyData = VecInit(clientExclusiveDirty, clientMigratoryDirty)
  ...
  def getReleaseType(incoming: Probe, meta: ClientMetadata): UInt = {
    ...
    val with_data = MuxLookup(incoming.p_type, releaseInvalidateData, Array(
      probeInvalidate -> Mux(VecInit(clientExclusiveDirty, clientMigratoryDirty).contains(meta.state),
